...
Every user story carries a `Status:` checkbox line. The Agent Builder reads every unchecked Must Have and builds them in one pass, ticking each box as that story lands green, then stops at the Must Have / Should Have boundary. Should / Could items only get built after an explicit PO promotion. **`backlog.md` is the plan and the progress ledger in one file.** (The scaffold is zipped to consultants, not transferred via git — the `[x]` tick is the only ledger.)
