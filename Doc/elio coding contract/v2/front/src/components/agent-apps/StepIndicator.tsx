...
interface StepIndicatorProps {
  ...
  readonly variant?: "pills" | "neo-pills" | "numbered" | "line";
...
export function StepIndicator({
  ...
  if (variant === "neo-pills") {
    return (
      <div
        className={cn(
          "inline-flex gap-1.5 p-1.5 rounded-lg bg-[rgba(255,255,255,0.2)] max-[720px]:w-full max-[720px]:overflow-x-auto dark:bg-[rgba(255,255,255,0.04)]",
          className,
        )}
        role="tablist"
      >
        {steps.map((step, index) => {
          ...
          const { isActive, isClickable } = getStepState(stepNumber);
          const isLocked = stepNumber > clickableUpTo && !isActive;
          ...
          return (
            <button
              ...
              className={cn(
                "inline-flex items-center min-h-10 px-4 rounded-xl text-base font-medium whitespace-nowrap transition-all max-[720px]:flex-[1_0_auto]",
                isActive &&
                  "bg-gradient-to-br from-[var(--blue-dark)] to-[var(--blue-primary)] text-white shadow-[0_10px_24px_rgba(0,89,223,0.28)]",
                !isActive &&
                  !isLocked &&
                  "text-[var(--grey-text)] hover:bg-[rgba(0,89,223,0.06)] hover:text-[var(--noir-onepoint)]",
                isLocked &&
                  "text-[var(--grey-text)] opacity-45 cursor-not-allowed",
              )}
            >
              {step.label}
            </button>
