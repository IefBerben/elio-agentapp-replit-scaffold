...
def _save_to_tempfiles(buffer: io.BytesIO, original_name: str) -> str:
    ...
    Args:
        buffer: In-memory buffer with the file content.
        original_name: Human-readable filename (e.g. ``report.docx``).
...
def markdown_to_docx(
    ...
    to produce a DOCX. This mirrors the Pandoc-free fallback path used
    in the main neo-back ``word_generator.py``.

    Args:
        markdown_text: Markdown-formatted content.
        title: Document title (added as a heading).
        subtitle: Optional subtitle added below the title.
    ...
    if subtitle:
        ...
        # Insert subtitle as first paragraph after the auto-generated title
...
def text_to_docx(text: str, title: str) -> str:
    ...
    Produces a document with a level-1 heading followed by the text content
    split into paragraphs. Useful for quick exports without Markdown formatting.

    Args:
        text: Plain text content (newlines preserved as paragraphs).
        title: Document title (level-1 heading).
...
def slides_to_pptx(
    ...
    A title slide is added automatically as the first slide.
...
def _replace_placeholders_in_paragraph(
    ...
    Args:
        paragraph: A python-docx or python-pptx paragraph object.
        placeholders: Key→value mapping.
        left_delim: Left delimiter (e.g. ``{{`` or ``<<``).
        right_delim: Right delimiter (e.g. ``}}`` or ``>>``).
    """
...
def fill_docx_template(
    ...
    Scans all paragraphs and table cells in the document for occurrences
    of ``{{key}}`` and replaces them with the corresponding value.

    Args:
        placeholders: Mapping of placeholder keys to replacement values.
        output_name: Base filename for the output (e.g. ``filled_report``).
        template_path: Path to the .docx template. Defaults to
            ``DOCX_TEMPLATE_OP`` (FR branded A4 format).
...
def fill_pptx_template(
    ...
    Mirrors the ``presentation_filler.py`` pattern from the main backend:
    scans all shapes on all slides for ``<<key>>`` text and replaces it
    with the corresponding value from the placeholders dict.

    Args:
        template_path: Path to the .pptx template file.
        placeholders: Mapping of placeholder keys to replacement values.
        output_name: Base filename for the output (e.g. ``filled_deck``).
